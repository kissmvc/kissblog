...
class KissApp {
	...
	public function __construct($config = null, &$page = null) {
		...
		$this->page->redirect = function($url, $inc_folder = true, $code = 302) {
	...
	public function redirect($url, $inc_folder = true, $code = 302) {
		header('Location: '.str_replace('//', '/', (($inc_folder ? '/'.$this->folder : '').'/'.$url)), true, $code);
		exit();
	}
